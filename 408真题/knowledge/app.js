(function () {
    ...
    function renderCard(item) {
      ...
      return `<article class="knowledge-card" id="${escapeHtml(item.id)}">
        ...
        <div class="card-details">
          ...
          <div class="card-actions"><a class="practice-link" href="practice.html?knowledge=${encodeURIComponent(item.id)}">进入章节练习 →</a><a class="truth-link" href="../index.html?subject=${encodeURIComponent(item.subject)}&search=${encodeURIComponent(item.keywords[0])}">在真题库中检索</a></div>
        </div>
      </article>`;
    }
  ...
  function setupPracticePage() {
    ...
    function render() {
      ...
      if (!item) return;
      const questions = (frequencies.get(item.id) || []).slice().sort((a, b) => a.year - b.year || Number(a.number) - Number(b.number));
      history.replaceState(null, "", `?knowledge=${encodeURIComponent(item.id)}`);
      intro.innerHTML = `<div class="practice-intro"><span class="eyebrow">${escapeHtml(item.subject)} · ${escapeHtml(item.chapter)}</span><h2>${escapeHtml(item.title)}</h2><p>${escapeHtml(item.summary)}</p><a class="back-knowledge" href="index.html#${escapeHtml(item.id)}">← 回到完整知识卡</a></div>`;
      generated.innerHTML = `<article class="generated-card"><span class="source-badge">教材同考法 · 改编巩固题</span><h2>先不看答案，独立完成</h2><div class="practice-prompt">${escapeHtml(item.practice.prompt)}</div><details class="answer"><summary>展开答案与推导</summary><div class="answer-content"><strong>答案：${escapeHtml(item.practice.answer)}</strong><p>${escapeHtml(item.practice.explanation)}</p></div></details></article>`;
      ...
      realList.innerHTML = questions.length ? questions.map((question) => {
        ...
        return `<article class="real-question" id="${escapeHtml(question.id)}"><div class="real-meta"><span>${question.year} 第 ${escapeHtml(question.number)} 题</span><span>${escapeHtml(question.kind)}</span><span>${escapeHtml(question.topic)}</span></div><div class="real-prompt">${prompt}</div>${options.length ? `<div class="real-options">${options.map((option) => `<div>${escapeHtml(optionText(option))}</div>`).join("")}</div>` : ""}<details class="answer real-answer"><summary>查看答案与解析</summary><div class="answer-content"><strong>${question.answer ? `答案：${escapeHtml(question.answer)}` : "主观题：按过程自评"}</strong><p>${escapeHtml(question.analysis)}</p><a class="back-knowledge" href="../index.html?question=${encodeURIComponent(question.id)}">回原真题页作答 →</a></div></details></article>`;
