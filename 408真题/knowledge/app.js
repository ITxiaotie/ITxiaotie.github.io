(function () {
  ...
  function setupPracticePage() {
    ...
    function render() {
      ...
      intro.innerHTML = `<div class="practice-intro"><span class="eyebrow">${escapeHtml(item.subject)} · ${escapeHtml(item.chapter)}</span><h2>${escapeHtml(item.title)}</h2><p>${escapeHtml(item.summary)}</p><div class="intro-actions"><a class="back-knowledge" href="index.html#${escapeHtml(item.id)}">← 回到完整知识卡</a><a class="exam-entry" href="exam.html?knowledge=${encodeURIComponent(item.id)}">用本知识点模拟考试 →</a></div></div>`;
      generated.innerHTML = `<article class="generated-card interactive-question" data-graded="false"><span class="source-badge">教材同考法 · 改编巩固题</span><h2>选择答案后提交</h2><div class="practice-prompt">${escapeHtml(item.practice.prompt)}</div>${renderAnswerOptions(item.practice.options, String(item.practice.correct))}<div class="answer-actions"><button class="check-answer" type="button" disabled>提交答案</button><button class="retry-answer" type="button" hidden>重新作答</button></div><div class="answer-feedback" hidden><strong></strong></div><div class="answer-panel" hidden><strong>标准答案：${escapeHtml(item.practice.answer)}</strong><p>${escapeHtml(item.practice.explanation)}</p></div></article>`;
